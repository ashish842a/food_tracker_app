...
import { getUserEntries, getEntriesByDate, getEntriesByMonth, getMonthlyExpense, getDailyExpenseBreakdown, getMealTypeExpense, deleteEntry, updateEntry, FoodEntry, MealType } from "@/lib/foodStore";
...
const Dashboard = () => {
  ...
  const reload = () => {
    if (user) setEntries(getUserEntries(user.id));
  };
  ...
  const monthlyExpense = useMemo(() => monthEntries.filter(e => e.price !== null).reduce((s, e) => s + (e.price || 0), 0), [monthEntries]);
  const totalMeals = entries.length;
  const daysTracked = useMemo(() => new Set(entries.map(e => e.date)).size, [entries]);

  const dailyBreakdown = useMemo(() => {
    const bd = getDailyExpenseBreakdown(user?.id || "", year, month);
    return Object.entries(bd).sort(([a], [b]) => a.localeCompare(b)).map(([date, amount]) => ({ date: date.slice(5), amount }));
  }, [user, year, month, entries]);

  const mealExpense = useMemo(() => {
    const me = getMealTypeExpense(user?.id || "", year, month);
    return Object.entries(me).filter(([_, v]) => v > 0).map(([name, value]) => ({ name, value }));
  }, [user, year, month, entries]);
  ...
  const handleEdited = (updated: FoodEntry) => {
    setEditEntry(null);
